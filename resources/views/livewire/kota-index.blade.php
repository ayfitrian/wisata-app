<div class="p-6 bg-white shadow-md rounded-lg max-w-4xl mx-auto">
    <h2 class="text-2xl font-bold mb-6 text-gray-800">🏙️ Kelola Kota</h2>
    ...
    <form wire:submit.prevent="submit" class="flex flex-col sm:flex-row items-center justify-center gap-4 mb-6">
        ...
        <button 
            type="submit" 
            class="bg-blue-600 hover:bg-blue-700 text-black px-4 py-2 rounded transition flex items-center gap-2">
        ...
        <table class="min-w-full bg-white border border-gray-300 rounded shadow-sm mx-auto">
            <thead>
                <tr class="bg-gray-100 text-gray-700">
                    ...
                    <th class="px-4 py-2 border-b text-center">Nama Kota</th>
            ...
            <tbody>
                @forelse ($kotas as $kota)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-2 border-b text-center">{{ $kota->id }}</td>
                        <td class="px-4 py-2 border-b text-center">{{ $kota->nama_kota }}</td>
                        <td class="px-4 py-2 border-b text-center space-x-2">
                            <button 
                                wire:click="edit({{ $kota->id }})" 
                                class="text-blue-600 hover:underline">
                                ✏️ Edit
                            </button>
                            <button 
                                wire:click="delete({{ $kota->id }})" 
                ...
                @empty
                    <tr>
                        <td colspan="3" class="text-center text-gray-500 py-4">Belum ada kota.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
